/**
 * LinkedIn Unified MCP Server - Main Entry Point
 *
 * This server provides a unified LinkedIn integration with:
 * - OAuth 2.0 authentication (no manual cookie management)
 * - Multi-tenant credential storage (HashiCorp Vault)
 * - 18 LinkedIn tools combining API, browser automation, and scraping
...
function createExpressServer(
        ...
        return res.status(400).send(`
          <html>
            ...
            <body style="font-family: Arial, sans-serif; padding: 50px; text-align: center;">
              ...
              <p>LinkedIn returned an error: <strong>${error}</strong></p>
  ...
  app.delete('/oauth/revoke', async (req: Request, res: Response) => {
    try {
      ...
      // Note: Actual revocation will be implemented by vault client
      logger.info('Credentials revocation requested', { tenantId });
...
async function main() {
  try {
    logger.info('Starting LinkedIn Unified MCP Server...');
    ...
    const vaultClient = new VaultClient(
      ...
      process.env.VAULT_NAMESPACE || 'linkedin-mcp'
    );
    ...
    const encryptionKey = process.env.COOKIE_ENCRYPTION_KEY || 'default-encryption-key-change-me';
    ...
    const httpServer = app.listen(port, () => {
      ...
      logger.info(`Authorization URL: http://localhost:${port}/oauth/authorize?tenant_id=YOUR_TENANT_ID`);
    ...
    registerTools(mcpServer, oauthManager, sessionManager);

    // 10. Start MCP server with stdio transport
    const transport = new StdioServerTransport();
    await mcpServer.connect(transport);

    logger.info('LinkedIn Unified MCP Server started successfully');
    logger.info('Server info:', {
      mcpServer: 'linkedin-unified v1.0.0',
      oauthEndpoint: `http://localhost:${port}/oauth/authorize`,
      transport: 'stdio',
      capabilities: 'OAuth 2.0, Multi-tenant, 18 LinkedIn tools'
    });

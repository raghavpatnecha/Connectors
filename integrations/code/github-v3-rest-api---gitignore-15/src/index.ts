/**
 * MCP Server: GitHub v3 REST API - gitignore
 * Category: code
 * Version: 1.1.4
 *
...
// API Client
class GitHubv3RESTAPIgitignoreClient {
   ...
   */
  async gitignoregetAllTemplates(params: {
...
const server = new Server(
  {
    name: "github-v3-rest-api---gitignore-15",
    version: "1.1.4",
  },
  {
    capabilities: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const client = new GitHubv3RESTAPIgitignoreClient(accessToken);

  try {
    switch (name) {
      case "gitignoregetAllTemplates": {
        const result = await client.gitignoregetAllTemplates(args as any);
        return {
          content: [
            {
              type: "text",
      ...
      case "gitignoregetTemplate": {
        const result = await client.gitignoregetTemplate(args as any);
        return {
          content: [
            {
              type: "text",
...
async function main() {
  ...
  console.error("GitHub v3 REST API - gitignore MCP Server running on stdio");
}
